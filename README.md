...
**TKTK: Describe app-template here.**
...
View the [sample copy spreadsheet](https://docs.google.com/spreadsheet/pub?key=0AsL6rgE-FlmBdDNpVVJrYWhmSjN5TWVtTTUtRGtLenc#gid=0).
...
def read_my_google_doc():
    ...
    get_document('0AsL6rgE-FlmBdDNpVVJrYWhmSjN5TWVtTTUtRGtLenc', file_path)
    data = Copy(file_path)
...
```
* * * * * ubuntu bash /home/ubuntu/apps/app_template/repository/cron.sh fab $DEPLOYMENT_TARGET cron_jobs.test
```
...
|--------|------|-----|-----|
|app-template|tweet|`location`||
|app-template|facebook|`location`||
|app-template|email|`location`||
|app-template|new-comment||
|app-template|open-share-discuss||
|app-template|close-share-discuss||
|app-template|summary-copied||
|app-template|featured-tweet-action|`action`|
|app-template|featured-facebook-action|`action`|
